...
void EnemySpawner::Update() {
    ...
    for (size_t i = 0; i < mEnemies.size(); ++i) {
        for (size_t j = i + 1; j < mEnemies.size(); ++j) {
            if (mEnemies[i]->CheckCollision(mEnemies[j])) {
                mEnemies[i]->Hit(mEnemies[j]);
                mEnemies[j]->Hit(mEnemies[i]);
            }
        }
    }

    // Remove enemies that are off the screen or destroyed
