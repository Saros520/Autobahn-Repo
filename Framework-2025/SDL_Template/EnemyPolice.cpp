    ...
    if (mChasing) {
        ...
        for (Enemy* enemy : mEnemySpawner->GetEnemies()) {
            if (enemy != this && enemy != nullptr) {
                ...
                if (verticalDistance < 360.0f && horizontalDistance < 150.0f) { // Adjust the distance thresholds as needed
                    Vector2 avoidDirection = (Position() - enemyPos).Normalized();
                    moveAmount += avoidDirection * speed * mTimer->DeltaTime();
                    speed = mBaseSpeed * 0.85f;
                    mAvoiding = true;
                }

                // Check if the enemy is vertically or diagonally between the police car and the player car
                if ((enemyPos.y < playerPos.y && enemyPos.y > Position().y && std::abs(enemyPos.x - Position().x) < 50.0f) ||
        ...
        // If the path is blocked, move horizontally to avoid the enemy vehicle
        if (pathBlocked) {
            if (Position().x < Graphics::SCREEN_WIDTH * 0.5f) {
                moveAmount = Vector2(speed * mTimer->DeltaTime(), 0.0f); // Move right
            }
            else {
                moveAmount = Vector2(-speed * mTimer->DeltaTime(), 0.0f); // Move left
            }
        }
        else if (!mAvoiding) {
            speed = mBaseSpeed;
